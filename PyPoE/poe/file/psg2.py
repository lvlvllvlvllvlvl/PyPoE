...
class GraphGroupNode(ReprMixin):
    ...
    def __init__(self, parent, passive_skill, radius, position, connections, splines):
        """
        Parameters
        ----------
        parent :  GraphGroup
        ...
        splines : list[int]
            these numbers appear to control the curvature of the corresponding connection
        """
        self.parent = parent
...
class PSGFile(AbstractFileReadOnly):
    ...
    def _read(self, buffer, *args, **kwargs):
        ...
        self.root_passives = []
        for _ in range(0, root_length):
            connection, curvature = struct.unpack_from("<II", data, offset=offset)
            self.root_passives.append(connection)
            if curvature:
                raise ValueError(
                    f"Unknown extra value {curvature} set for root passive {connection} at offset {offset}"
                )
            offset += 8
        ...
        for i in range(0, group_length):
            ...
            for j in range(0, passive_length):
                ...
                if connections_length > 1000:
                    warnings.warn(
                        f"There are unrealistically many connections ({connections_length})"
                        f" for passive {rowid} at {group_offset}.\n"
                        f"Skipping to prevent allocating too much memory"
                    )
                    offset += 8 * connections_length
                else:
                    connections = []
                    splines = []
                    for _ in range(0, connections_length):
                        connection, curvature = struct.unpack_from("<Ii", data, offset=offset)
                        connections.append(connection)
                        splines.append(curvature)
                        offset += 8
